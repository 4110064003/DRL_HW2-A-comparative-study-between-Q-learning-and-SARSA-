...
# Be tolerant of positional/misquoted invocation from CMD:
# If RepoUrl doesn't look like a Git remote, treat it as a commit message
# and reset RepoUrl/Branch back to safe defaults.
if ($RepoUrl -notmatch '^(https?://|git@)') {
    if ($CommitMessage -eq "chore: update HW2 project") {
        if ($Branch -and $Branch -ne "main") {
            $CommitMessage = "$RepoUrl $Branch".Trim()
        } else {
            $CommitMessage = $RepoUrl
        }
    }
    $RepoUrl = "https://github.com/4110064003/DRL_HW2-A-comparative-study-between-Q-learning-and-SARSA-.git"
    $Branch = "main"
}
